        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {messages.length === 0 && (
            ...
            >
              ...
              {/* Main hero content */}
              <div className={cn("text-left pl-3 sm:pl-4 scale-[0.92] origin-top-left", trainerName && trainerAvatarUrl && "md:ml-16 lg:ml-0")}>
              ...
              <div className="bg-card border border-border rounded-2xl p-6 shadow-lg text-center">
                ...
                <div className="space-y-2">
                  <Button className="w-full bg-gradient-brand hover:opacity-90 text-white font-semibold" onClick={() => handleUpgradeCheckout('lifetime')} disabled={isCheckingOut}>
                    {isCheckingOut ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Loading...</> : <>Exam Season Pass – <span className="line-through opacity-60 mr-1">£39.99</span>{productSlug?.startsWith('gcse-') ? '£17.99' : '£16.99'}</>}
                  </Button>
                  <Button variant="outline" className="w-full" onClick={() => handleUpgradeCheckout('monthly')} disabled={isCheckingOut}>
                    Monthly – {productSlug?.startsWith('gcse-') ? '£6.99' : '£8.99'}/mo
                  </Button>
                </div>
