...
import type { AnnotatedCandidate, Course, MatchInputs } from "./types";
import { REACH_LABEL } from "./tiers";
...
/**
 * Client helper: ask the server to match and order courses for these inputs.
 * Returns the candidate courses, the model's best-fit order, and the total
 * matches (for "top N of M" disclosure). Throws on failure so the caller can
 * show an error; the edge function itself degrades to an empty order on model
 * failure.
 */
export async function requestOrder(inputs: MatchInputs): Promise<MatchResult> {
  const { data, error } = await supabase.functions.invoke("uni-rank", {
    body: { inputs },
  });
  if (error) throw new Error(`Ranking request failed: ${error.message}`);
  const result = (data ?? {}) as Partial<MatchResult>;
  return {
    order: result.order ?? [],
...
/**
 * Client helper: ask the server for one-line reasons for the shortlisted course
 * ids. Throws on failure so the caller can show cards without reasons.
 */
export async function requestReasons(
  inputs: MatchInputs,
  courseIds: string[],
): Promise<RankedResult[]> {
  const { data, error } = await supabase.functions.invoke("uni-reasons", {
    body: { inputs, courseIds },
  });
  if (error) throw new Error(`Reasons request failed: ${error.message}`);
  return (data as { reasons: RankedResult[] }).reasons;
}
