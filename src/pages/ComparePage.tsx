...
export const ComparePage = () => {
  ...
  // Live prices keyed by product slug (monthly_price / lifetime_price from the products table)
  ...
  // Close the board dropdown on any outside click
  useEffect(() => {
  ...
  // Load dynamic products + build the live price map (all active products)
  useEffect(() => {
    const loadDynamic = async () => {
      ...
      eq('active', true);
      if (data) {
        // Price map keyed by slug — covers both legacy and dynamic products
  ...
  const boardLabel = fmtBoard(examBoard);
  ...
  // Feature cards
  const markerName = ({ economics: 'Essay marker', psychology: 'Essay marker', 'computer-science': 'Long answer marker' } as Record<string, string>)[subject] || 'Answer marker';
  const diagram = ({
  ...
  // Founder strip product id (DB-driven)
  const founderProductId = getDynamicProduct()?.id || (currentSlug ? PRODUCT_IDS[currentSlug] : undefined);

  // Testimonials marquee (verbatim from the design)
  const T = [
    { pre: 'A*AI helped me go ', hl: 'from a C in my summer mocks to getting predicted an A', post: ' in November.', image: matanImage, name: 'Matan G', role: 'Year 13 Student' },
    { pre: 'The 25-marker banded marking gave my essay 23/25 - ', hl: 'exactly what my teacher gave it', post: '.', image: ryanDaviesImage, name: 'Ryan Davies', role: 'Year 13' },
    { pre: 'In those final weeks before the exam it was a lifesaver, helping me ', hl: 'achieve A*s in Econ, Maths and Politics', post: '.', image: '/lovable-uploads/8e3350f3-3dd2-4e1f-b88a-c678f461e79d.png', name: 'Sina Naderi', role: 'BA Economics, Cambridge' },
    { pre: 'Got my ', hl: 'first ever 12/12 on a 12 marker', post: " and then 10/10 today with A*AI's help", image: kathyKouImage, name: 'Kathy Kou', role: 'Year 12' },
    ...
    { pre: 'The feedback was ', hl: 'more detailed than any standard examiner marking', post: '. My teacher recommended it to my classmates.', image: louisYungImage, name: 'Louis Yung', role: 'Year 12' },
    { pre: 'Diagrams generated instantly, ', hl: 'essay feedback I could actually use', post: '.', image: '', name: 'Natas Bubelis', role: 'Year 13' },
    { pre: 'Bro... ', hl: 'I wish I found this sooner', post: ". It's perfect.", image: oliverMobolajiImage, name: 'Oliver Mobolaji', role: 'Year 13' },
  ].map(t => ({
    ...t,
  ...
  return (
    <div className="compare-landing" style={{ minHeight: '100vh', background: '#ffffff', color: '#18181b', fontFamily: font }}>
      ...
      <SEOHead
        ...
        description="Compare A* AI plans for A-Level revision. Economics (Edexcel, AQA, CIE), Computer Science (OCR) & Physics (OCR). Free to start."
        ...
        <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: navPad, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(10px)', border: navBorder, borderRadius: navRadius, boxShadow: navShadow, transition: 'padding 0.35s cubic-bezier(0.22, 0.61, 0.36, 1), border-radius 0.35s cubic-bezier(0.22, 0.61, 0.36, 1), box-shadow 0.35s cubic-bezier(0.22, 0.61, 0.36, 1), border-color 0.35s ease' }}>
          ...
          <div className="cmp-nav-center" style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)' }}>
            <div style={{ display: 'flex', gap: '24px', paddingBottom: '1px', borderBottom: '1px solid #e6e4ec' }}>
              ...
              <div className="nav-link" onClick={() => { navigate('/university'); setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 100); }} style={{ fontSize: '14px', paddingBottom: '5px', color: '#71717a', cursor: 'pointer' }}>Uni Apps</div>
        ...
        <div style={{ position: 'relative', zIndex: 10, maxWidth: '1152px', margin: '0 auto' }}>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      try {
                        const map = JSON.parse(localStorage.getItem('preferred-exam-boards') || '{}');
                        setExamBoard(map[s] || LEGACY_DEFAULT_BOARD[s] || '');
                      } catch { setExamBoard(LEGACY_DEFAULT_BOARD[s] || ''); }
                    }}
          ...
          <div className="cmp-subject-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    {boardsForSubject.map((b) => {
                      ...
                      return (
                        <button key={b} className="board-item" onClick={() => { setExamBoard(b); setBoardMenuOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', border: 'none', background: activeBoard ? 'hsla(263, 70%, 50%, 0.06)' : 'transparent', borderRadius: '8px', fontFamily: font, fontSize: '14px', fontWeight: 500, color: '#18181b', cursor: 'pointer', textAlign: 'left' }}>
                          <span style={{ display: 'inline-flex', width: '14px', color: P }}>{activeBoard ? '✓' : ''}</span>{fmtBoard(b)}
                        </button>
                      );
                    })}
                  </div>
              ...
              <span style={{ fontSize: '13px', color: '#a1a1aa' }}>A-Level · Full spec coverage</span>
            </div>
      ...
      {founderProductId && (
        <DynamicFounderSection
          ...
          fallbackSubject={LEGACY_SUBJECTS.includes(subject) && subject !== 'psychology' ? subject : undefined}
          fallbackExamBoard={examBoard}
        />
      )}
      ...
      <section id="testimonials" className="cmp-section-pad" style={{ padding: '80px 0', overflow: 'hidden', background: 'linear-gradient(to bottom, #ffffff, hsla(263, 60%, 50%, 0.035) 40%, #ffffff)' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px', padding: '0 32px' }}>
          <h2 style={{ fontSize: '40px', fontWeight: 700, letterSpacing: '-0.02em', margin: 0, color: '#18181b' }}>Loved by sixth formers <span style={{ position: 'relative', display: 'inline-block', whiteSpace: 'nowrap' }}>across the UK<svg viewBox="0 0 300 14" preserveAspectRatio="none" style={{ position: 'absolute', left: 0, bottom: '-8px', width: '100%', height: '14px', overflow: 'visible' }}><path d="M4 9 C 60 2, 130 3, 158 6 C 200 10, 255 9, 296 4" stroke={P} strokeWidth="4" fill="none" strokeLinecap="round" opacity="0.55"></path></svg></span></h2>
        </div>
      ...
      <footer style={{ padding: '64px 32px', textAlign: 'center', borderTop: '1px solid rgba(228,228,232,0.6)' }}>
        <div style={{ maxWidth: '896px', margin: '0 auto' }}>
          <p style={{ color: '#71717a', margin: '0 0 24px 0', fontSize: '16px' }}>Your AI-powered A-Level revision coach</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '16px', marginBottom: '24px', color: '#71717a', fontSize: '16px' }}>
            <Link to="/compare">Plans</Link><span>•</span>
